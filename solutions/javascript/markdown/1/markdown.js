...
function replaceDelmiter(markdown, delimiter, tag) {
  const pattern = new RegExp(`${delimiter}(.+)${delimiter}`);
  const replacement = `<${tag}>$1</${tag}>`;
  return markdown.replace(pattern, replacement);
}

function replace__(markdown) {
  return replaceDelmiter(markdown, '__', 'strong');
}

function replace_(markdown) {
  return replaceDelmiter(markdown, '_', 'em');
}

function parseText(markdown, list) {
  const parsedText = replace_(replace__(markdown));
  if (list) {
    return parsedText;
  } else {
